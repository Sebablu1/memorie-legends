  ...
  return async function salirDeSalaEnEspera({ uid, codigo }) {
    ...
    return db.runTransaction(async (tx) => {
      ...
      if (esCreador) {
         ...
         *
         * `jugadores`, `jugadoresNombres` y `jugadoresLuce` son paralelas: el
         * asiento de cada quien es su posición en las tres. Sacando al que se
         * va de dos de ellas y no de la tercera, todos los que estaban detrás
         * quedan con la cara del de adelante — y la mesa se ve perfecta, con
         * cuatro jugadores y cuatro caras que no son las suyas.
         *
         * Esto ya pasó de verdad, cuando la tercera lista se reconstruía acá
         * sin los retratos. Es la falla más fea de las que puede tener esto,
         * porque no rompe nada visible. La cuida
         * `pruebas/retratos-en-red.mjs`, que audita este archivo y el de las
         * salas.
         *
         * `jugadoresRetratos` es el nombre viejo de la tercera, de cuando
         * llevaba sólo la cara: hay salas abiertas creadas con él.
         */
        const indice = jugadores.indexOf(uid);
        const nombres = [...(sala.jugadoresNombres ?? [])];
        const luces = [...(sala.jugadoresLuce ?? sala.jugadoresRetratos ?? [])];
        nombres.splice(indice, 1);
        luces.splice(indice, 1);
        tx.update(refSala, {
